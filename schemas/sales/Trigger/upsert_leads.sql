...
RETURNS TRIGGER AS
	...
	-- delete old entries
    DELETE FROM sc.leads_test
	WHERE "Id" IN (SELECT "Id" FROM zoho."Leads" WHERE "Modified Time" > (SELECT max(last_event_on) FROM sc.leads_test)); 

	-- insert new leads 
    INSERT INTO sc.leads_test
        SELECT 
		    "Id"::bigint,
